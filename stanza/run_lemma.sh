...
if [[ "$lang" == "vi" || "$lang" == "fro" ]]; then
    python3 -m stanza.models.identity_lemmatizer --data_dir $LEMMA_DATA_DIR --train_file $train_file --eval_file $eval_file \
        --output_file $output_file --gold_file $gold_file --lang $short
else
    python3 -m stanza.models.lemmatizer --data_dir $LEMMA_DATA_DIR --train_file $train_file --eval_file $eval_file \
        --output_file $output_file --gold_file $gold_file --lang $short --num_epoch $num_epoch --mode train $args
    python3 -m stanza.models.lemmatizer --data_dir $LEMMA_DATA_DIR --eval_file $eval_file \
